...
import { Activity, TrendingUp, DollarSign, Zap, Wallet, Eye } from 'lucide-react';
...
export const LiveDashboard: React.FC<LiveDashboardProps> = ({ showReports, onNavigateToWorkflowRun }) => {
  ...
  // Mock balance data
  const dexBalances = [
    { chain: 'ethereum', symbol: 'ETH', balance: '2.5734', usdValue: 9006.9, vaultAddress: '0x742d...5b1' },
    { chain: 'ethereum', symbol: 'USDC', balance: '5420.30', usdValue: 5420.3, vaultAddress: '0x742d...5b1' },
    { chain: 'polygon', symbol: 'MATIC', balance: '8450.12', usdValue: 6760.1, vaultAddress: '0x981c...2a7' },
    { chain: 'bsc', symbol: 'BNB', balance: '12.87', usdValue: 7722.0, vaultAddress: '0x523a...9f2' },
  ];

  const cexBalances = [
    { exchange: 'binance', symbol: 'BTC', balance: '0.1234', usdValue: 5432.1 },
    { exchange: 'binance', symbol: 'ETH', balance: '3.8976', usdValue: 13641.6 },
    { exchange: 'okx', symbol: 'USDT', balance: '2100.50', usdValue: 2100.5 },
  ...
  return (
    <div className="p-4 space-y-4">
        ...
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white border border-gray-200'} rounded-lg p-4`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl font-bold text-blue-400">${stats.todayProfit.toFixed(2)}</p>
            </div>
        ...
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white border border-gray-200'} rounded-lg p-4`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl font-bold text-orange-400">{stats.activeTrades}</p>
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white border border-gray-200'} rounded-lg p-4`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-right">
              ...
              <div className="text-xl font-bold text-blue-400">${dexBalances.reduce((sum, b) => sum + b.usdValue, 0).toLocaleString()}</div>
            </div>
          </div>
              ...
              <div key={idx} className={`flex items-center justify-between p-2 ${isDark ? 'bg-gray-700' : 'bg-gray-50'} rounded text-xs`}>
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-400 capitalize">{t(`dashboard.balance.${balance.chain}`)}</div>
                  </div>
                </div>
                <div className={`text-xs font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>${balance.usdValue.toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white border border-gray-200'} rounded-lg p-4`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-right">
              <div className="text-sm text-gray-400">{t('common.total')}</div>
              <div className="text-xl font-bold text-purple-400">${cexBalances.reduce((sum, b) => sum + b.usdValue, 0).toLocaleString()}</div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2">
              ...
              <div key={idx} className={`flex items-center justify-between p-2 ${isDark ? 'bg-gray-700' : 'bg-gray-50'} rounded text-xs`}>
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-400 capitalize">{t(`dashboard.balance.${balance.exchange}`)}</div>
                  </div>
                </div>
                <div className={`text-xs font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>${balance.usdValue.toLocaleString()}</div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white border border-gray-200'} rounded-lg p-4`}>
          <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'} mb-4`}>{t('dashboard.tradePerformance')}</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
          ...
          <table className="w-full">
            ...
            <tbody>
              {trades.slice(0, 10).map(trade => (
                <tr key={trade.id} className={`border-b border-gray-700 hover:${isDark ? 'bg-gray-700' : 'bg-gray-50'} transition-colors`}>
                  <td className="py-2 px-3 text-gray-300 text-sm">{trade.timestamp.toLocaleTimeString()}</td>
                  <td className="py-2 px-3 text-gray-300 text-sm">{trade.strategy}</td>
                  <td className="py-2 px-3 text-gray-300 text-sm">{trade.pair}</td>
                  <td className="py-2 px-3 text-gray-300 text-sm">${trade.amount.toFixed(2)}</td>
                  <td className={`py-2 px-3 font-medium text-sm ${trade.profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                    ${trade.profit.toFixed(2)}
                  </td>
                  <td className="py-2 px-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs ${
                        trade.status === 'success' ? 'bg-green-600 text-green-200' : 'bg-red-600 text-red-200'
                      }`}
                    >
                      {trade.status}
                    </span>
                  ...
                  <td className="py-2 px-3">
                    <button
                      ...
                      className={`inline-flex items-center space-x-1 px-2 py-1 text-xs rounded-md transition-colors ${
                        isDark ? 'bg-blue-600 hover:bg-blue-700 text-blue-100' : 'bg-blue-100 hover:bg-blue-200 text-blue-700'
                      }`}
