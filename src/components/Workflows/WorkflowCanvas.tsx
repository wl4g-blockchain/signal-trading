...
export const Canvas: React.FC<CanvasProps> = ({ nodes, connections, onNodesChange, onConnectionsChange, onDeleteNode }) => {
  ...
  const handleDragMove = useCallback(
    (event: React.MouseEvent) => {
      ...
      const updatedNodes = nodes.map(node => (node.id === draggedNode ? { ...node, position: { x: newX, y: newY } } : node));
  ...
  const handleMouseMoveWhileConnecting = useCallback(
    (event: MouseEvent) => {
  ...
  const handleConnectionStart = useCallback(
    ...
    [scale, panOffset, handleMouseMoveWhileConnecting, handleMouseUpWhileConnecting]
  );
  ...
  const handleConnectionEnd = useCallback(
    (targetNodeId: string) => {
      ...
      const existingConnection = connections.find(conn => conn.source === isConnecting.nodeId && conn.target === targetNodeId);
  ...
  return (
    <>
      ...
      {/* 节点配置弹窗 */}
      {configNode && <NodeConfigModal node={configNode} onSave={handleNodeUpdate} onClose={() => setConfigNode(null)} />}
    </>
  );
};
